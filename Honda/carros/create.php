...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (empty($marca) || empty($modelo) || empty($ano) || empty($cor) || !isset($_POST['valor'])) {
        ...
        try {
            $stmt = $pdo->prepare("INSERT INTO carros (marca, modelo, ano, quilometragem, valor, cor, imagem_url) VALUES (:marca, :modelo, :ano, :quilometragem, :valor, :cor, :imagem_url)");
            $stmt->execute([
                'marca' => $marca,
                'modelo' => $modelo,
                'ano' => $ano,
            ...
            header("Location: list.php?success=" . urlencode("Carro adicionado com sucesso."));
            exit;
...
<div class="space-y-6">
    <div class="flex items-center justify-between pb-5 border-b border-slate-800/80">
        <div>
            <h1 class="text-2xl font-bold tracking-tight text-white font-sans">Cadastrar Carro</h1>
            <p class="text-slate-400 text-sm mt-1">Adicione um novo veículo ao inventário da concessionária</p>
        </div>
