...
try {
    ...
    die("Erro ao carregar dados do carro: " . $e->getMessage());
}
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (empty($marca) || empty($modelo) || empty($ano) || empty($cor) || !isset($_POST['valor'])) {
        ...
        $error = "O valor do veículo deve ser maior que zero.";
    } else {
        if (empty($imagem_url)) {
            $imagem_url = "https://images.unsplash.com/photo-1533473359331-0135ef1b58bf?auto=format&fit=crop&q=80&w=600";
        }
        try {
            $stmt = $pdo->prepare("UPDATE carros SET marca = :marca, modelo = :modelo, ano = :ano, quilometragem = :quilometragem, valor = :valor, cor = :cor, imagem_url = :imagem_url WHERE id = :id");
            $stmt->execute([
                'marca' => $marca,
                'modelo' => $modelo,
                'ano' => $ano,
            ...
            $error = "Erro ao atualizar veículo: " . $e->getMessage();
        }
    }
}
?>
<div class="space-y-6">
    <div class="flex items-center justify-between pb-5 border-b border-slate-800/80">
        <div>
            <h1 class="text-2xl font-bold tracking-tight text-white font-sans">Editar Carro</h1>
        ...
        <form action="edit.php?id=<?php echo $id; ?>" method="POST" class="space-y-6">
            <div class="grid grid-cols-1 sm:grid-cols-2 gap-6">
                <div>
                    ...
                    <select name="marca" id="marca" required
                        ...
                        <option value="Honda" <?php echo $car['marca'] === 'Honda' ? 'selected' : ''; ?>>Honda</option>
                        <option value="Fiat" <?php echo $car['marca'] === 'Fiat' ? 'selected' : ''; ?>>Fiat</option>
                        <option value="Volkswagen" <?php echo $car['marca'] === 'Volkswagen' ? 'selected' : ''; ?>>Volkswagen</option>
                        <option value="Mercedes-Benz" <?php echo $car['marca'] === 'Mercedes-Benz' ? 'selected' : ''; ?>>Mercedes-Benz</option>
                        <option value="Toyota" <?php echo $car['marca'] === 'Toyota' ? 'selected' : ''; ?>>Toyota</option>
                        <option value="Chevrolet" <?php echo $car['marca'] === 'Chevrolet' ? 'selected' : ''; ?>>Chevrolet</option>
                    </select>
                </div>
                <div>
                    <label for="modelo" class="block text-xs font-semibold uppercase tracking-wider text-slate-400 mb-2">Modelo</label>
                    <input type="text" name="modelo" id="modelo" required placeholder="Civic EXL"
            ...
            <div class="grid grid-cols-1 sm:grid-cols-3 gap-6">
                ...
                <div>
                    ...
                    <input type="text" name="cor" id="cor" required placeholder="Preto Cristal"
            ...
            <div class="grid grid-cols-1 sm:grid-cols-2 gap-6">
                <div>
                    ...
                    <input type="number" step="0.01" name="valor" id="valor" required placeholder="149900.00"
                ...
                <div>
                    ...
                    <input type="url" name="imagem_url" id="imagem_url" placeholder="https://..."
                           value="<?php echo htmlspecialchars($_POST['imagem_url'] ?? $car['imagem_url']); ?>"
                           class="w-full bg-slate-950/60 border border-slate-800 hover:border-slate-700 focus:border-red-500 rounded-xl py-2.5 px-4 text-slate-200 placeholder-slate-600 focus:outline-none transition text-sm">
                </div>
            </div>
