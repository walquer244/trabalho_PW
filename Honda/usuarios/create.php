    ...
    if (empty($nome) || empty($email) || empty($senha)) {
        ...
        try {
            ...
            if ($stmt->fetch()) {
                ...
                $stmtInsert = $pdo->prepare("INSERT INTO usuarios (nome, email, senha, nivel_acesso) VALUES (:nome, :email, :senha, :nivel_acesso)");
                $stmtInsert->execute([
                    'nome' => $nome,
                    'email' => $email,
                    'senha' => $hash,
                ...
                header("Location: list.php?success=" . urlencode("Usuário cadastrado com sucesso."));
                exit;
            }
