...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (empty($nome) || empty($funcao) || empty($data_admissao) || empty($data_nascimento) || !isset($_POST['salario'])) {
        ...
        try {
            $stmt = $pdo->prepare("UPDATE funcionarios SET nome = :nome, funcao = :funcao, data_admissao = :data_admissao, data_nascimento = :data_nascimento, salario = :salario WHERE id = :id");
            $stmt->execute([
                'nome' => $nome,
                'funcao' => $funcao,
            ...
            header("Location: list.php?success=" . urlencode("Dados do funcionário atualizados com sucesso."));
            exit;
        } catch (PDOException $e) {
...
<div class="space-y-6">
    <div class="flex items-center justify-between pb-5 border-b border-slate-800/80">
        <div>
            ...
            <p class="text-slate-400 text-sm mt-1">Atualize as informações cadastrais do funcionário</p>
        </div>
            ...
            <div class="grid grid-cols-1 sm:grid-cols-2 gap-6">
                <div>
                    ...
                    <input type="text" name="nome" id="nome" required
                           value="<?php echo htmlspecialchars($_POST['nome'] ?? $func['nome']); ?>"
                ...
                <div>
                    ...
                    <input type="text" name="funcao" id="funcao" required
                           value="<?php echo htmlspecialchars($_POST['funcao'] ?? $func['funcao']); ?>"
                           class="w-full bg-slate-950/60 border border-slate-800 hover:border-slate-700 focus:border-red-500 rounded-xl py-2.5 px-4 text-slate-200 placeholder-slate-600 focus:outline-none transition text-sm">
                </div>
            </div>
            <div class="grid grid-cols-1 sm:grid-cols-3 gap-6">
                <div>
                    ...
                    <input type="date" name="data_admissao" id="data_admissao" required
                           value="<?php echo htmlspecialchars($_POST['data_admissao'] ?? $func['data_admissao']); ?>"
                ...
                <div>
                    ...
                    <input type="date" name="data_nascimento" id="data_nascimento" required
                           value="<?php echo htmlspecialchars($_POST['data_nascimento'] ?? $func['data_nascimento']); ?>"
                           class="w-full bg-slate-950/60 border border-slate-800 hover:border-slate-700 focus:border-red-500 rounded-xl py-2.5 px-4 text-slate-200 focus:outline-none transition text-sm text-slate-400">
                </div>
                <div>
                    ...
                    <input type="number" step="0.01" name="salario" id="salario" required placeholder="3200.00"
                           value="<?php echo htmlspecialchars($_POST['salario'] ?? $func['salario']); ?>"
                           class="w-full bg-slate-950/60 border border-slate-800 hover:border-slate-700 focus:border-red-500 rounded-xl py-2.5 px-4 text-slate-200 placeholder-slate-600 focus:outline-none transition text-sm">
                </div>
            </div>
